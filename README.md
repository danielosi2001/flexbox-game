...
> **מצב נוכחי:** מוטמע רק צד ה-Front — `index.html`, `css/`, `assets/`. קבצי
> `js/` עדיין ריקים, ולכן מסך הפתיחה נראה כמו שצריך אבל "התחלת המשימה" לא
> יגיב והלוח יישאר ריק עד שצד הלוגיקה ייכנס.
